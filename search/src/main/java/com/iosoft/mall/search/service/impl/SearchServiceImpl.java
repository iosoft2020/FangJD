...
import com.iosoft.mall.search.feign.ProductFeignService;
import com.iosoft.mall.search.service.SearchService;
import com.iosoft.mall.search.vo.AttrResponseVo;
...
public class SearchServiceImpl implements SearchService {
    ...
    private SearchResult bulidSearchResult(SearchParam searchParam, SearchResponse searchResponse) {
        ...
        //2.3 总页码
        Integer totalPages = (int) total % EsConstant.PRODUCT_PAGESIZE == 0 ? (int) total / EsConstant.PRODUCT_PAGESIZE
        ...
        if (attrs != null && attrs.size() > 0) {
            List<SearchResult.NavVo> navVos = attrs.stream().map(attr -> {
                ...
                String replace = queryString.replace("&attrs=" + attr, "").replace("attrs=" + attr + "&", "")
                        .replace("attrs=" + attr, "");
